...
function error(message) {
    ...
    window.alert("ERROR [" + new Date() + "]: " + message);
}
...
function appendParameterToUrl(url, parameterName, parameterValue) {
    ...
    const parameter = !parameterValue || parameterValue.length === 0
        ? encodeURIComponent(parameterName)
        : encodeURIComponent(parameterName) + "=" + encodeURIComponent(parameterValue);

    const questionSignPos = url.indexOf("?");
    const sharpPos = url.indexOf("#");
    ...
    if (questionSignPos === -1 && sharpPos === -1) {
        ...
        const resultUrl = url.substring(0, questionSignPos + 1) + parameter;
...
Codeforces = new function () {
    ...
    this.startTabBlinking = function (tabId) {
        const title = document.title;

        const blinkInterval = setInterval(() => {
            document.title = document.title === title ? "*****" : title;
        }, 500);
        info("setInterval: Started blinking [" + blinkInterval + ", title: " + title + "].");

        if ("BroadcastChannel" in window) {
            const channel = new BroadcastChannel("codeforces-announcement-close");
            channel.onmessage = (event) => {
                info("clearInterval: Received message: " + JSON.stringify(event.data) + ".");
    ...
    this.stopTabBlinking = function (blinkInterval) {
        info("clearInterval: Stopped blinking [" + blinkInterval + "].");
        clearInterval(blinkInterval);

        if ("BroadcastChannel" in window) {
            const channel = new BroadcastChannel("codeforces-announcement-close");
            channel.postMessage({"name": "codeforces-announcement-close"});
            info("postMessage: Sent message.");
        }
    }

    this.showAnnouncements = function (json, locale) {
        if (!Array.isArray(json) || json.length === 0) {
            console.error('Expected json to be an array of positive length');
            return;
        }
        ...
        const div =
            document.getElementsByClassName("notificationTextCleaner")[0];
        if (!div) {
        ...
        for (let i = 0; i < json.length; ++i) {
            ...
            div.innerHTML = json[i].text
                || (locale === "ru" ? json[i].russianText : json[i].englishText)
                || json[i].russianText
                || json[i].englishText;

            const name = json[i].name
                || (locale === "ru" ? json[i].russianName : json[i].englishName)
                || json[i].russianName
                || json[i].englishName;
        ...
        if (messages.length !== 0) {
            requestAnimationFrame(() => {
                // const blinkInterval = Codeforces.startTabBlinking();
                messages.forEach(message => {
                    alert(`${message.name}\n----------\n\n${message.text}`);
                });
                // Codeforces.stopTabBlinking(blinkInterval);
            });
        }
    };
    ...
    this.showMessage = function (message) {
        $.jGrowl(Codeforces.sanitize(message), {position: 'bottom-right', life: 10000});
        CodeforcesIframe.updatePopups();
    };
...
//            window.localStorage[key] = defaultValue;
    ...
    this.setupContestTimes2 = function (contestDataPageUrl, elem) {
        console.info("setupContestTimes...")
        if (!elem) {
            elem = $("html");
        }
    ...
    this.setupContestTimes = function (contestDataPageUrl, elem) {
        ...
        // Collect all contestIds from the elements
        const contestElements = elem.find(".contest-time");
        const contestIds = [...new Set(contestElements.map(function() {
            return $(this).attr("contestId");
        }).get())];
        ...
        info(JSON.stringify(contestIds));

        // Make a single AJAX call
        $.post(contestDataPageUrl, {
            action: "getContestTimes",
            contestIds: contestIds
        }, function (result) {
            if (result["success"] === "true" && result["items"]) {
                result["items"].forEach(function(item) {
                    info(JSON.stringify(item));
                    if (item["success"] === "true") {
                        const contestId = item["contestId"];
                        const contestTimeHref = item["contestTimeHref"];
                        const contestTimeHtml = item["contestTimeHtml"];

                        console.log(contestId, contestTimeHref, contestTimeHtml);

                        const contestElement= contestElements.filter(`[contestId="${contestId}"]`);
                        contestElement.attr("href", contestTimeHref);
                        contestElement.html(contestTimeHtml);
                    }
                });
...
$(document).ready(function () {
    // Check if there is at least one image with class 'resource'
    if ($("img.resource").length > 0) {
        ...
        $("body").append(`
            <div id="imagePopup" style="
                ...
                align-items: center;
                z-index: 1000;">
                <img id="popupImage" src="" alt="" style="
                    max-width: 80vw;
                    max-height: 80vh;
                    border-radius: 8px;
